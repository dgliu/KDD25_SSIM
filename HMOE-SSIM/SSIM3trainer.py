...
import numpy as np
from modules import hmoe_SSIM3
from sklearn import metrics
from utils import trainUtils
import random
import pickle

parser = argparse.ArgumentParser(description="hmoe trainer")
...
parser.add_argument("--share_units", type=int, nargs='+', default=[128], help="share mlp layer size")
parser.add_argument("--share_dropout", type=float, default=[0.0], help="share mlp dropout rate")
parser.add_argument("--tower_units", type=int, nargs='+', default=[16], help="tower_units layer size")
parser.add_argument("--tower_dropout", type=float, default=[0.0], help="tower_dropout rate")
parser.add_argument("--expert_dim", type=int, default=64, help="expert_dim")
parser.add_argument("--expert_num", type=int, default=8, help="expert_num")

# device information
parser.add_argument("--cuda", type=int, choices=range(-1, 8), default=5, help="device info")
...
parser.add_argument("--lambda1", type=float, default=0.2, help="lambda")
...
class Trainer(object):
    def __init__(self, opt):
        ...
        self.model = hmoe_SSIM3.HMoE(opt['model_opt'])
        self.model_opt = hmoe_SSIM3.HMoE_OPT()
        self.model_eval = hmoe_SSIM3.HMoE_Eval()
        ...
        self.opt = opt
        self.model_init_dir = 'model3_init.h5'
        self.model_mask_dir = 'model3_mask.h5'
    ...
    def train(self, epochs):
        self.model = hmoe_SSIM3.HMoE(self.opt['model_opt'])
        ...
        self.model_mask = hmoe_SSIM3.HMoE(self.opt['model_opt'])
        ...
        for epoch_idx in range(int(epochs)):
            ...
            if val_auc > cur_auc:
                cur_auc = val_auc
                self.model.save_weights("save3.h5")
            else:
                self.model.load_weights("save3.h5")
                early_stop = True
                te_auc, te_loss = self.evaluate_test("test")
                for s in self.model.scenes:
                    self.logger.info("Early stop at epoch {epoch:d}|Test AUC{d:}: {te_auc:.6f}, Test Loss{d:}:{te_loss:.6f}".
...
def main():
    ...
    model_opt = {
        ...
        "share_units": args.share_units, "share_dropout": args.share_dropout, "tower_units": args.tower_units, "tower_dropout": args.tower_dropout,
        "expert_dim": args.expert_dim, "expert_num": args.expert_num, "l2_reg": args.l2, "mlp_dropout": args.mlp_dropout, "use_bn": args.mlp_bn, "mlp_dims": args.mlp_dims,
...
if __name__ == "__main__":
    """
    python SSIM3trainer.py --dataset ali-ccp 
    """
    main()

...
parser.add_argument("--tower_units", type=int, nargs='+', default=[32], help="tower_units layer size")
...
class Trainer(object):
    def __init__(self, opt):
        ...
        self.model = hmoe.HMoE(opt['model_opt'])
        self.model_opt = hmoe.HMoE_OPT()
        self.model_eval = hmoe.HMoE_Eval()
    ...
    def train(self, epochs):
        ...
        for epoch_idx in range(int(epochs)):
            ...
            if val_auc > cur_auc:
                cur_auc = val_auc
                self.model.save_weights('save2.h5')
            else:
                self.model.load_weights('save2.h5')
                early_stop = True
        ...
        if not early_stop:
            ...
            for s in self.model.scenes:
                self.logger.info(
                    "Final Test AUC{d:}:{te_auc:.6f}, Test Loss{d:}:{te_loss:.6f}".format(d=s, te_auc=te_auc[s], te_loss=te_loss[s]))
...
def main():
    ...
    model_opt = {
        ...
        "expert_dim": args.expert_dim, "expert_num": args.expert_num, "l2_reg": args.l2
    }

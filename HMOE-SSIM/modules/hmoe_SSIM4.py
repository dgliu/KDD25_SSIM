...
class HMoE(Model):
    def __init__(self, param_dict):
        ...
        self.domain4_mask = tf.keras.layers.Dense(units=1, input_shape=(self.embed_dims[-1],))
    ...
    def compute(self, x, d):
        ...
        if self.ticket:
            ...
            m4 = self.domain4_mask(hyper_output)
            ...
            if self.topk is None:
                ...
                smask4 = tf.cast(tf.greater(m4, 0), tf.float32)

                d_eq_0 = tf.cast(tf.equal(d, 0), tf.float32)
                d_eq_1 = tf.cast(tf.equal(d, 1), tf.float32)
                d_eq_2 = tf.cast(tf.equal(d, 2), tf.float32)
                d_eq_3 = tf.cast(tf.equal(d, 3), tf.float32)

                smask = smask1 * d_eq_0 + smask2 * d_eq_1 + smask3 * d_eq_2 + smask4 * d_eq_3
                ...
                m = m1 * tf.cast(tf.equal(d, 0), tf.float32) + \
                    ...
                    m4 * tf.cast(tf.equal(d, 3), tf.float32)
            ...
            smask4 = tf.sigmoid(self.temp * m4)

...
def feature_example(label, feature, domain):
    feature_des = {
...
class DataTransform(object):
    def __init__(self, dataset_path1, dataset_path2, stats_path, store_stat=False, seed=2021):
        self.data_path1 = dataset_path1
        self.data_path2 = dataset_path2
    ...
    def _read(self, name=None, header=None, sep=None, label_index="", domain_id=""):
        ...
        self.traindata = pd.read_table(self.data_path1, sep=sep)
        self.testdata = pd.read_table(self.data_path2, sep=sep)
        self.traindata.columns = name
        self.testdata.columns = name
        self.data = pd.concat([self.traindata, self.testdata], ignore_index=True)
        ...
        self.num_fields = self.data.shape[1] - 2

        self.field_name = self.data.columns.values.tolist()
        assert self.num_fields == len(self.field_name) - 2
    ...
    def random_split(self, ratio=[]):
        ...
        # train_data = shuffle(self.train_data, random_state=self.seed)
        # val_data = self.test_data.sample(frac=ratio[0], replace=False,
        #                             axis=0, random_state=self.seed)
        # test_data = self.test_data[~self.test_data.index.isin(val_data.index)]
    ...
    def transform_tfrecord(self, data, record_path, flag, records=5e6, label_index="", domain_id=""):
        ...
        while records * part <= data.shape[0]:
            ...
            for index, row in tmp_data.iterrows():
                ...
                # if oov:
                # continue
                tf_writer.write(feature_example(label, feature, domain))
                pbar.update(1)
                instance_num += 1
            tf_writer.close()
            pbar.close()

...
# mask information
parser.add_argument("--m_lr", type=float, help="mask network learning rate", default=1e-4)
parser.add_argument("--r_lr", type=float, help="retrain learning rate", default=3e-4)
parser.add_argument("--r_l2", type=float, help="retrain L2 regularization", default=3e-6)
...
parser.add_argument("--thre", type=float, default=0.49, help="thre")
parser.add_argument("--lambda1", type=float, default=1.0, help="lambda")
parser.add_argument("--topk", type=float, default=[0.3], help="sclect_rate")
...
class Trainer(object):
    def __init__(self, opt):
        ...
        self.network = SSIM4mask.getModel(opt["model"], opt["model_opt"]).to(self.device)
        self.criterion = torch.nn.BCEWithLogitsLoss(reduction='none')
        self.optim = SSIM4mask.getOptim(self.network, opt["optimizer"], self.lr, self.m_lr, self.l2)
        ...
        self.opt = opt
        self.model_init_dir = 'model_init_s4'
    ...
    def train_on_batch(self, label, data, domain, retrain=False):
        ...
        logit1, logit2, logit3, logit4, smask = self.network(data, domain)
        ...
        if not retrain:
            main_loss = torch.mean(logloss1 * torch.eq(domain, 0).type(torch.long) * torch.eq(smask, 0).type(torch.long) +
                                   ...
                                   logloss4 * torch.eq(domain, 3).type(torch.long) * torch.eq(smask, 0).type(torch.long)+
                                   (logloss1 * smask + logloss2 * smask + logloss3 * smask + logloss4 * smask) / 4)
    ...
    def eval_on_batch(self, data, domain):
        ...
        with torch.no_grad():
            ...
            logit1, logit2, logit3, logit4, _ = self.network(data, domain)
            logit = logit1 * torch.eq(domain, 0).type(torch.long) + logit2 * torch.eq(domain, 1).type(torch.long) \
                    + logit3 * torch.eq(domain, 2).type(torch.long) + logit4 * torch.eq(domain, 3).type(torch.long)
    ...
    def search(self):
        ...
        torch.save({
            ...
            'domain4_mask_state_dict': self.network.domain4_mask.state_dict()
        }, 'mask_params_s4.pth')
    ...
    def train(self, epochs, retrain_id=0):
        ...
        self.opt["model_opt"]["mlp_dropout"] = 0.3
        print("mlp_dropout:", self.opt["model_opt"]["mlp_dropout"])
        self.network = SSIM4mask.getModel(self.opt["model"], self.opt["model_opt"]).to(self.device)
        self.network.load_state_dict(torch.load(self.model_init_dir))

        checkpoint = torch.load('mask_params_s4.pth')
        ...
        self.network.domain4_mask.load_state_dict(checkpoint['domain4_mask_state_dict'])
        ...
        cur_auc = 0.0
        early_stop = False
        self.optim = SSIM4mask.getOptim(self.network, "adam", self.r_lr, self.m_lr, self.r_l2)[:1]
